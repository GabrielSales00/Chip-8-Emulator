...
Screen * newScreen(const char * title, int height, int width, int textureWidth, int textureHeight) {
    ...
    screen->window = SDL_CreateWindow(title, 0, 0, width, height, SDL_WINDOW_SHOWN);
    screen->renderer = SDL_CreateRenderer(screen->window, -1, SDL_RENDERER_ACCELERATED);
    screen->texture = SDL_CreateTexture(
        screen->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, textureWidth, textureHeight);
...
void update(Screen* screen, void const* buffer, int pitch) {
    SDL_UpdateTexture(screen->texture, NULL, buffer, pitch);
